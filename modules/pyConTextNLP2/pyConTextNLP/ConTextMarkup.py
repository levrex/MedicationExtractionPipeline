...
class ConTextMarkup(nx.DiGraph):
    ...
    def markItems(self, items, mode="target"):
        ...
        if not items:
            return
        for item in items:
            terms = self.markItem(item, ConTextMode=mode)
            self.add_nodes_from(terms, category=mode)
    ...
    def markItem(self, item, ConTextMode="target", ignoreCase=True):
        ...
        for i in _iter:
            tag_0 = tagObject(item,
                              ...
                              tagid=create_tag_id(),
                              scope=self.getScope())
            l_keys= re.findall('\$(\S*)', tag_0.getCapturingCode())
            d_features = {}
            for n in range(len(i.groups())):
                d_features[l_keys[n]] = i.group(n+1)
            tag_0.setCapture(d_features)
    ...
    def pruneModifierRelationships(self):
        ...
        for modifier in list(modifiers):
            modified_by = list(self.successors(modifier))
            if modified_by and len(list(modified_by)) > 1:
                minm = min([(modifier.dist(mod_by), mod_by) for mod_by in modified_by])
                edgs = list(self.edges(modifier))
