...
def performancePlot(df, title):
    ...
    for xe, ye in zip(list(df.keys()),list(df.values())):
        ix = 0
        for i in ye:
            plt.scatter(xe, i, marker=markers[ix], s=100, edgecolors='black', linewidths=.4, color=colors[ix])
            ix += 1
    
    legend_elements = [Line2D([0], [0], marker=markers[0], color=colors[0], label='Ppv', markersize=15, markeredgecolor='black'),
                       Line2D([0], [0], marker=markers[1], color=colors[1], label='Npv', markersize=15, markeredgecolor='black'),
                       Line2D([0], [0], marker=markers[2], color=colors[2], label='Sensitivity', markersize=15, markeredgecolor='black'),
                       Line2D([0], [0], marker=markers[3], color=colors[3], label='Specificity', markersize=15, markeredgecolor='black'),
                       Line2D([0], [0], marker=markers[4], color=colors[4], label='Accuracy', markersize=15, markeredgecolor='black')]       
...
def append_roc_curve(title, lbl, fpr, tpr, plt):
    """
...
def plot_roc_curve(estimator, title, X, y):
    ...
    ax1.set_xlim([0, 1])
    ax1.set_ylim([0, 1])
    ax1.set_ylabel('Sensitiviteit (TPR)')
    ax1.set_xlabel('1 - Specificiteit (FPR)')
